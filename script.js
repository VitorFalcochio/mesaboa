...
function registerServiceWorker() {
  ...
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('service-worker.js').catch(() => {
      // The app still works as a regular browser app if local SW registration is blocked.
    });
  });
}
...
window.addEventListener('hashchange', () => setPage(location.hash.slice(1) || 'home', false));
